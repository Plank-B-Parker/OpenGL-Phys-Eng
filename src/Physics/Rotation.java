...
public class Rotation implements PhysicsAttrib{
	...
	static MathsSoup temps = new MathsSoup("Axis");
	
	public Rotation(Vect3d centre, Quaternion orientation, Quaternion _orientation, Mat3d Basis, Mat3d _Basis, Mat3d MomentOfInertia) {
		this.centre = centre;
	...
	private void updateVerlet(double dt) {
		temps.startOfMethod();
		//////////////////////
		Vect3d.increment(angMomentum, torque, dt/2);
		
		Mat3d oldBasis = temps.getMat();
		Mat3d _oldBasis = temps.getMat();
	...
	private void updateClosedLoop(double dt) {
		if(torque.lengthSq() > 0) correctE = actualE;
		
		if(actualE > correctE) updateImplicit2(dt);
		else updateVerlet(dt);
		
		if(Math.abs(1 - orientation.lengthSq()) > orientationTolerance*orientationTolerance) orientation.normalise();
	}
	
	private void updateImplicit1(double dt) {
		temps.startOfMethod();
		//////////////////////
	...
	private void updateExplicit(double dt) {
		temps.startOfMethod();
		/////////////////////
	...
	private void updateImplicit2(double dt) {
		temps.startOfMethod();
		//////////////////////
